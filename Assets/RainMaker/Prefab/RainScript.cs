...
namespace DigitalRuby.RainMaker
{
    public class RainScript : BaseRainScript
    {
        ...
        private void UpdateRain()
        {
            ...
            {
                Camera rainCamera = ResolveCamera();
                ...
                if (FollowCamera)
                {
                    ...
                    RainFallParticleSystem.transform.position = rainCamera.transform.position;
                    RainFallParticleSystem.transform.Translate(0.0f, RainHeight, RainForwardOffset);
                    RainFallParticleSystem.transform.rotation = Quaternion.Euler(0.0f, rainCamera.transform.rotation.eulerAngles.y, 0.0f);
                    if (RainMistParticleSystem != null)
                    {
                        var s2 = RainMistParticleSystem.shape;
                        s2.shapeType = ParticleSystemShapeType.Hemisphere;
                        Vector3 pos = rainCamera.transform.position;
